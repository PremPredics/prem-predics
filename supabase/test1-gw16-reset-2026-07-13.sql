...
-- GW16 starts on 15 July 2026 at 15:00 Europe/London.
...
declare
  ...
  abby_user_id uuid;
  gw1_id bigint;
  gw16_base_date date := date '2026-07-15';
begin
  ...
  select
    ...
    'test_gw16_reset_deck_2026_07_13'
  from (
  ...
  select
    ...
    'test_gw16_reset_super_cards_2026_07_13'
  ...
  cross join lateral generate_series(
    1,
    case when super_cards.id = 'super_draw' then 5 else 1 end
  ) as copies(copy_number);
  ...
  select
    ...
    starter.source
  from (
    select members.user_id, power_card.id as card_id, 'test_gw16_reset_starter_power_2026_07_13' as source
    from (values (vas_user_id), (abby_user_id)) as members(user_id)
    cross join lateral (
      select id
      from public.card_definitions
    ...
    select members.user_id, curse_card.id as card_id, 'test_gw16_reset_starter_curse_2026_07_13' as source
    from (values (vas_user_id), (abby_user_id)) as members(user_id)
    cross join lateral (
      select id
      from public.card_definitions
