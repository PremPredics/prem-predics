-- Prem Predics live migration: Curse target limits.
-- Safe scope: installs validation only. It does not edit cards, leagues, users or existing effects.

begin;

-- Remove the superseded three-Gameweek same-target cooldown if it was installed.
drop trigger if exists active_card_effects_enforce_curse_target_cooldown
on public.active_card_effects;
drop function if exists public.enforce_curse_target_cooldown();
...
begin
  ...
  -- Serialise both the target cap and each player's within-Gameweek target order.
...
before insert or update of competition_id, card_instance_id, card_id, season_id, gameweek_id, start_gameweek_id, end_gameweek_id, played_by_user_id, target_user_id, status
on public.active_card_effects
...
where t.tgrelid = 'public.active_card_effects'::regclass
  ...
  and t.tgname = 'active_card_effects_enforce_curse_target_rules';
