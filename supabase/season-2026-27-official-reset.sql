...
--
-- This version intentionally avoids temporary/staging tables so it runs reliably
...
fixture_seed(gameweek_number, sort_order, home_team_name, away_team_name, kickoff_local) as (
  values
...
gameweek_locks as (
  select
    fixture_seed.gameweek_number as number,
    min(fixture_seed.kickoff_local::timestamp at time zone 'Europe/London') - interval '90 minutes' as star_man_locks_at
  from fixture_seed
  group by fixture_seed.gameweek_number
)
insert into public.gameweeks (season_id, number, star_man_locks_at)
select target_season.id, gameweek_locks.number, gameweek_locks.star_man_locks_at
from target_season
cross join gameweek_locks
...
with raw_promoted_seed(display_name, team_name, nationality, height_cm) as (
  values
...
seed_base as (
  select
    raw_promoted_seed.*,
    regexp_replace(lower(extensions.unaccent(coalesce(raw_promoted_seed.display_name, ''))), '[^a-z0-9]+', '', 'g') as norm_name,
    regexp_split_to_array(trim(coalesce(raw_promoted_seed.display_name, '')), '\s+') as words
  from raw_promoted_seed
),
promoted_seed as (
  select
    seed_base.display_name,
    seed_base.team_name,
    seed_base.nationality,
    seed_base.height_cm,
    seed_base.norm_name,
    seed_base.words[1] as first_name,
    case
      when array_length(seed_base.words, 1) is null then null
      when array_length(seed_base.words, 1) = 1 then seed_base.words[1]
      when surname_particle.particle_pos is not null then array_to_string(seed_base.words[surname_particle.particle_pos:array_length(seed_base.words, 1)], ' ')
      else seed_base.words[array_length(seed_base.words, 1)]
    end as last_name
  from seed_base
  left join lateral (
    select min(i) as particle_pos
    from generate_subscripts(seed_base.words, 1) as i
    where i > 1
      and lower(seed_base.words[i]) in ('van', 'de', 'den', 'der', 'del', 'da', 'di', 'dos', 'du', 'le', 'la')
  ) surname_particle on true
)
...
select
  promoted_seed.display_name,
  promoted_seed.first_name,
  promoted_seed.last_name,
  promoted_seed.last_name,
  promoted_seed.last_name,
  teams.id,
  'squad_player',
  promoted_seed.nationality,
  promoted_seed.height_cm,
  true
from promoted_seed
join public.teams teams on teams.name = promoted_seed.team_name
where not exists (
  select 1
  from public.players players
  where regexp_replace(lower(extensions.unaccent(coalesce(players.display_name, ''))), '[^a-z0-9]+', '', 'g') = promoted_seed.norm_name
);
...
matched_promoted_players as (
  select
    promoted_seed.*,
    teams.id as team_id,
    players.id as player_id,
    row_number() over (
      partition by promoted_seed.norm_name
      order by (players.team_id = teams.id) desc, players.is_active desc, players.created_at desc
    ) as rn
  from promoted_seed
  join public.teams teams
    on teams.name = promoted_seed.team_name
  join public.players players
    on regexp_replace(lower(extensions.unaccent(coalesce(players.display_name, ''))), '[^a-z0-9]+', '', 'g') = promoted_seed.norm_name
)
update public.players players
set
  display_name = matched_promoted_players.display_name,
  first_name = matched_promoted_players.first_name,
  last_name = matched_promoted_players.last_name,
  surname = matched_promoted_players.last_name,
  scrabble_name = matched_promoted_players.last_name,
  team_id = matched_promoted_players.team_id,
  squad_status = 'squad_player',
  nationality = matched_promoted_players.nationality,
  height_cm = matched_promoted_players.height_cm,
  is_active = true
from matched_promoted_players
where players.id = matched_promoted_players.player_id
  and matched_promoted_players.rn = 1;
...
update public.players players
set is_active = false
from canonical_promoted_players
where regexp_replace(lower(extensions.unaccent(coalesce(players.display_name, ''))), '[^a-z0-9]+', '', 'g') = canonical_promoted_players.norm_name
  and players.id <> canonical_promoted_players.player_id;
...
current_premier_league_teams(team_name, short_name) as (
  values
('Arsenal', 'ARS'),
  ('Aston Villa', 'AVL'),
  ('Bournemouth', 'BOU'),
...
join current_premier_league_teams current_teams on current_teams.team_name = teams.name
cross join target_season
